...
OPTION_NAMES = ['mode', 'speed_pendown', 'speed_penup', 'accel', 'pen_pos_down', 'pen_pos_up',
                    'pen_rate_lower', 'pen_rate_raise', 'handling', 'random_start', 'hiding',
                    'reordering', 'auto_rotate', 'homing', 'report_time','utility_cmd', 'dist',
                    'layer', 'copies', 'page_delay', 'preview', 'rendering', 'model',
                    'penlift', 'port', 'port_config', 'webhook', 'webhook_url',
                    'digest', 'progress']
...
def effect_parse(effect, svg_input):
    ''' `effect` is an `inkex.Effect` object, e.g. `NextDrawControl`
    ...
    try: # try assuming config is a filename
        ...
        if len(config) > 3 and config[-3:] == ".py" and ose.errno == errno.ENOENT:
            ...
            with warnings.catch_warnings():
                ...
                # since technically this is importing "nextdrawcore.nextdraw_conf" twice, it would generate a warning, but we can ignore it
